...
public:
    ...
    void operator()(double endTime){
        while (time < endTime){
            const auto& validReactionTimes = findFastestReactionTime();
            auto newTime = validReactionTimes.second;
            auto fastestReaction = validReactionTimes.first;
            
            time += newTime;
            fastestReaction.operator()();
            this->addState();
        }
        std::cout << *this;
    }
